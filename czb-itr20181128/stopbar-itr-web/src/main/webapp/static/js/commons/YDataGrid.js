var YDataGrid = function(config){
		...
		var Action = {
			...
			'excel': actionUrl.excel||'dataToExcel.do'
		}
		...
		var Handler = {
			...
			excel: function(callback){
					Form.excel.attr('action',Action.excel);
					YiYa.saveForm(Form.excel,function(data){
							 //Win.edit.dialog('close');
						     //Form.excel.resetForm();
						     //回调函数
							if(jQuery.isFunction(callback)){
								callback(data);
							}
			...
			add: function(callback){
				...
				document.getElementById("fitemLoginPWD").disabled="";
			},
			//edit 按钮事件
			edit:  function(callback){
				...
				document.getElementById("fitemLoginPWD").disabled="disabled";
			},
			//刷新Grid 数据
		...
		var bar_excel = { id:'btnexcel',
				text:'导出',
				iconCls:'icon-print',
				btnType:'excel',
				handler:Events.excel
			   };
		var toolbarConfig = [bar_add,bar_edit,bar_remove,bar_excel];
		var getToolbar = function (){
			...
			if (dataGrid.toolbar != undefined && dataGrid.toolbar.length > 0) {
				for ( var i = 0; i < dataGrid.toolbar.length; i++) {
					...
					if(bar.btnType=='excel'){
						tbars.push({id:bar.id || bar_excel.id,text:bar.text || bar_excel.text ,iconCls: bar.iconCls || bar_excel.iconCls,btnType: bar.btnType || bar_excel.btnType,handler:bar.handler || bar_excel.handler});
						continue;
					}
