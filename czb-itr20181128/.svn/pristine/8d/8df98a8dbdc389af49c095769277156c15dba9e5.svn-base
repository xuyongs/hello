...
/**
 * mysql元数据查询
 * （详细的字段的解释，请查看JDBC中的DatabaseMetaData的API）
 */
public class MetaDataUtils {
     ...
     */
    public static void MySqlDataType2(Connection conn, String tablename) throws SQLException, IOException, ClassNotFoundException {
        ...
        try {
            ...
            for (int i = 1; i <= rsmd.getColumnCount(); i++) {
                ...
                int type = rsmd.getColumnType(i);       // 获得指定列的数据类型，该类型值对应 java.sql.Types 类中的类型
...
//        printResultSet(rs);
        return processResultSet(rs, COLUMN_NAME, COLUMN_DATA_TYPE,
            COLUMN_TYPE_NAME, COLUMN_SIZE,
            COLUMN_DECIMAL_DIGITS, COLUMN_DEF,
            COLUMN_REMARKS, COLUMN_IS_NULLABLE,
     ...
     * @throws java.sql.SQLException
     */
    public static List<Map<String, String>> getPrimaryKeysMetaData(Connection conn, String tableName) throws SQLException {
        DatabaseMetaData metaData = conn.getMetaData();
...
//        printResultSet(rs);
    ...
    public static void main(String[] args) {
        ...
        try {
            Connection conn = getConnection("com.mysql.jdbc.Driver", "jdbc:mysql://112.74.114.2:3306/ct?useUnicode=yes&characterEncoding=UTF-8", "root", "123456");
            String tableName = "select * from user_info a";
...
//            generateInsertSql(conn, tableName, false);
            tablesMetaData = getIndexMetaData(conn, tableName);
