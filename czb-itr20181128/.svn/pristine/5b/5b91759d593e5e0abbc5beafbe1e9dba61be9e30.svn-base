...
import static com.agent.wechat.sdk.WXPayConstants.USER_AGENT;
...
public class WXPayRequest {
     ...
     * @throws Exception
     */
    public static String httpsRequest(String urlStr,  String data) throws Exception {
        BasicHttpClientConnectionManager connManager;
        ...
        RequestConfig requestConfig = RequestConfig.custom().setSocketTimeout(10*1000).setConnectTimeout(6*1000).build();
        httpPost.setConfig(requestConfig);

        StringEntity postEntity = new StringEntity(data, "UTF-8");
        httpPost.addHeader("Content-Type", "text/xml");
        httpPost.addHeader("User-Agent", USER_AGENT + " " + WeChatConfig.MchId);
        httpPost.setEntity(postEntity);
        HttpResponse httpResponse = httpClient.execute(httpPost);
        HttpEntity httpEntity = httpResponse.getEntity();
        return EntityUtils.toString(httpEntity, "UTF-8");
