			...
			public void updateEpWhite() {
				...
				if (parkWhiteListModel.getState().equals("0") && parkWhiteListModel.getCarportId() != null) {
					...
					// 为短租车
						...
						CarportInfoModel selectById = carportInfoService.selectById(parkWhiteListModel.getCarportId());
						SimpleDateFormat sdfhms = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
						try {
							updateEpBytime(parkWhiteListModel,selectById.getEffTime(),sdfhms.parse(selectById.getOpenTime()));
						} catch (ParseException e) {
